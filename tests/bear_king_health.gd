...
func test_encounter_configuration_and_patrol() -> void:
	...
	expect(
		is_equal_approx(harness.animation_length(bear_king, HURT_ANIMATION), 0.4),
		"BearKing keeps its hurt-immunity presentation timing"
	)
	expect(
		is_equal_approx(harness.animation_length(bear_king, SKILL_ANIMATION), 0.95),
		"BearKing keeps its earthquake presentation timing"
	)
	expect(
		is_equal_approx(harness.animation_length(bear_king, DEAD_ANIMATION), 1.0),
	...
	expect(harness.is_playing(patrol_bear_king, IDLE_ANIMATION), "BearKing initializes idling")
	expect(
		is_equal_approx(patrol_bear_king.global_position.x, start_x),
		"BearKing stays still during its configured idle"
	)
	await create_timer(0.2).timeout
	expect(harness.is_playing(patrol_bear_king, RUN_ANIMATION), "BearKing enters its run presentation")
	expect(patrol_bear_king.global_position.x < start_x, "BearKing begins its patrol leftward")
	expect(not harness.is_facing_right(patrol_bear_king), "BearKing faces its patrol direction")
...
func test_scaled_environment_wall_reversal() -> void:
	...
	expect(harness.is_facing_right(bear_king), "Scaled BearKing faces away from the wall")
...
func test_damage_faces_attacker_and_interrupts_earthquake() -> void:
	...
	expect(not harness.is_facing_right(bear_king), "BearKing faces an attacker on its left")
	expect(
	...
	expect(not harness.is_facing_right(bear_king), "Ignored damage does not change BearKing facing")
	...
	expect(harness.is_facing_right(bear_king), "BearKing faces an attacker on its right")
	await create_timer(0.3).timeout
...
func test_paused_death_outcome_and_cleanup() -> void:
	...
	for hit_index in EXPECTED_HEALTH - 1:
		await harness.deliver_hit(bear_king)
	...
	var hurt_event_count: Array[int] = [0]
	harness.instantiate_passive_player(
		bear_king.global_position + EARTHQUAKE_OFFSET,
		func() -> void: hurt_event_count[0] += 1
	)
	await harness.physics_frames(3)
	await create_timer(0.05).timeout
	expect(harness.is_playing(bear_king, SKILL_ANIMATION), "BearKing can earthquake before a lethal hit")
	...
	expect(death_outcome_count[0] == 1, "BearKing emits one boss death outcome")
	...
	expect(harness.is_playing(bear_king, DEAD_ANIMATION), "Lethal damage starts BearKing death presentation")
	expect(
		not harness.is_playing(bear_king, EARTHQUAKE_CAST_ANIMATION),
	...
	expect(hurt_event_count[0] == 0, "Dead BearKing cannot produce a delayed earthquake impact")
	expect(
		harness.animation_position(bear_king, DEAD_ANIMATION) >= 0.65,
	...
	expect(death_outcome_count[0] == 1, "BearKing death outcome remains singular after removal")
	paused = false
